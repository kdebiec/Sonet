    ...
    mStateTokenServer(sts), mNotify(notify), mRsMsgs(msgs), mRsPeers(peers), mRsIdentity(identity), mUnreadMsgNotify(unread), mMtx("ChatHandler::mMtx")
{
    ...
    addResourceHandler("send_message_via_pgp", this, &ChatHandler::handleSendMessageViaPGP);
...
void ChatHandler::handleMessages(Request &req, Response &resp)
{
...

    {
        ...
        if(mit == mMsgs.end())
        {
            ...
            resp.setFail("chat with id=\""+chat_id+"\" not found");
            return;
        }
...
void ChatHandler::handleSendMessageViaPGP(Request &req, Response &resp)
{
    std::string pgp_id;
    std::string msg;
    ...
    if (pgp.isNull())
    {
        resp.setFail("pgp_id is invalid");
        return;
    }

    std::list<RsPeerId> sslIds;
    rsPeers->getAssociatedSSLIds(pgp, sslIds);
    ...
    else
    {
        ...
        if (onlineIds.size() == 1)
        {
            ...
            id = ChatId(onlineIds.front());
        }
        else if(onlineIds.size() > 1)
        {
            int count = 0;
            for(std::list<RsPeerId>::iterator it = onlineIds.begin(); it != onlineIds.end(); ++it)
            {
                if(mRsMsgs->sendChat(ChatId(*it), msg))
                    count++;
            }
            if(count > 0)
                resp.setOk();
            else
                resp.setFail("failed to send messages");
            return;
        }
    }
