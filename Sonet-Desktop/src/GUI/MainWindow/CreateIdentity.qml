...
PopupBase {
    ...
    function createIdentity(name) { var jsonData = {callback_name: "createidentity_get_flickable_grid_mode", name: name, pgp_linked: true }; rsApi.request("/identity/create_identity", JSON.stringify(jsonData), function(par) { if(JSON.parse(par.response).data.name === dialog.name.text) dialog.close() })}
